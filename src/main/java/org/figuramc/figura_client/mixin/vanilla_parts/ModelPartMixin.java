package org.figuramc.figura_client.mixin.vanilla_parts;
    ...
    @Unique private void figuraCustomRender(PoseStack poseStack, VertexConsumer vertexConsumer, int i, int j, int k) {
        if (this.visible) {
            ...
            for (ModelPart part : this.children.values()) {
                VanillaPart vanillaPart = ((ModelPartMixin) (Object) part).vanillaPart;
                if (vanillaPart != null) {
                    currentPart = currentComponent.vanillaPartToScriptPart.get(vanillaPart); // Update currentPart
                    if (currentPart != null) {
                        ((ModelPartMixin) (Object) part).figuraCustomRender(poseStack, vertexConsumer, i, j, k);
                        continue;
                    }
                }
